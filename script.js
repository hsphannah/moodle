document.addEventListener('DOMContentLoaded', () => {
                    ...
                    container.innerHTML += `
                        <div class="course-card">
                            <div class="course-card-header">
                                ...
                                <button class="btn-icon" data-action="delete-course" data-course-id="${course.id}" title="Excluir Curso"><span class="material-symbols-outlined">delete</span></button>
                            </div>
    ...
    const handleSaveCourse = async () => {
        ...
        const url = editingCourseId ? `http://localhost:3000/api/cursos/${editingCourseId}` : 'http://localhost:3000/api/cursos';
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorResult.error || 'Erro ao salvar o curso.');
            }
            renderCourses();
            closeCourseModal();
        } catch (error) {
    ...
    const handleEditCourse = async (courseId) => {
        try {
            ...
            if (!response.ok) {
                const errorResult = await response.json();
                throw new Error(errorResult.error || 'Curso não encontrado.');
            }
            const result = await response.json();
    ...
    const handleDeleteCourse = async (courseId) => {
        ...
        try {
            ...
            if (!response.ok) {
                const errorResult = await response.json();
                throw new Error(errorResult.error || 'Erro ao excluir o curso.');
            }
            renderCourses();
        } catch (error) {
    ...
    const handleViewCourseDetails = async (courseId) => {
        ...
        try {
            const [courseResponse, studentsResponse] = await Promise.all([
                fetch(`http://localhost:3000/api/cursos/${courseId}`),
                fetch(`http://localhost:3000/api/cursos/${courseId}/alunos`, { headers: { 'Authorization': `Bearer ${getToken()}` } })
            ]);
    ...
    const renderAlunos = async () => {
        ...
        if (!container) return;
        try {
            const response = await fetch('http://localhost:3000/api/alunos', { headers: { 'Authorization': `Bearer ${getToken()}` } });
            const result = await response.json();
    ...
    const handleEditAluno = async (alunoId) => {
        ...
        try {
            const response = await fetch(`http://localhost:3000/api/alunos/${alunoId}`, { headers: { 'Authorization': `Bearer ${getToken()}` } });
            if (!response.ok) {
                const errorResult = await response.json();
                throw new Error(errorResult.error);
            }
            ...
            document.getElementById('aluno-curso').value = ""; // Opcional: não preenchemos o curso na edição para permitir nova inscrição
    ...
    const renderLessons = async (courseId) => {
        ...
        try {
            const response = await fetch(`http://localhost:3000/api/cursos/${courseId}/aulas`, { headers: { 'Authorization': `Bearer ${getToken()}` } });
    ...
    const handleSaveAula = async () => {
        ...
        if (conteudoArquivo && (tipo === 'PDF' || tipo === 'Imagem')) {
            formData.append('conteudo', conteudoArquivo);
        } else {
            formData.append('conteudo', conteudoTexto);
        }

        if (!titulo || !tipo || !currentCourseIdForLessons || (!conteudoTexto && !conteudoArquivo)) {
            alert('Por favor, preencha todos os campos obrigatórios.');
            return;
        }
        
        const url = editingAulaId ? `http://localhost:3000/api/aulas/${editingAulaId}` : 'http://localhost:3000/api/aulas';
    ...
    const handleEditAula = async (aulaId) => {
        try {
            const response = await fetch(`http://localhost:3000/api/aulas/${aulaId}`, { headers: { 'Authorization': `Bearer ${getToken()}` } });
            ...
            document.getElementById('aula-tipo').dispatchEvent(new Event('change')); // Dispara o evento para mostrar/esconder o input certo
            if (aulaToEdit.tipo !== 'PDF' && aulaToEdit.tipo !== 'Imagem') {
    ...
    document.addEventListener('click', (event) => {
        ...
        if (openModalButton) {
            ...
            if (modalId === 'modal-novo-aluno') {
                ...
                // Reseta o modal para o estado de "Adicionar" antes de abrir
                closeAulaModal();
                // Simula o evento change para garantir que o input correto seja exibido
        ...
        if (actionButton) {
            ...
            if (action === 'save-aula') handleSaveAula();
            if (action === 'edit-aula') handleEditAula(aulaId);
            if (action === 'delete-aula') handleDeleteAula(aulaId);
        }
    });
    ...
    async function loadPage(page) {
        ...
        try {
            ...
            if (page === 'conteudo') initializeContentPage();
        } catch (error) {
