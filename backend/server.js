...
const db = new sqlite3.Database('./db.sqlite', (err) => {
    ...
    db.serialize(() => {
        ...
        db.run(`CREATE TABLE IF NOT EXISTS inscricoes (id INTEGER PRIMARY KEY AUTOINCREMENT, aluno_id INTEGER NOT NULL, curso_id INTEGER NOT NULL, UNIQUE(aluno_id, curso_id))`);
        ...
        db.run(`CREATE TABLE IF NOT EXISTS aulas (id INTEGER PRIMARY KEY AUTOINCREMENT, titulo TEXT NOT NULL, tipo TEXT NOT NULL, conteudo TEXT NOT NULL, curso_id INTEGER NOT NULL)`);
        db.run(`CREATE TABLE IF NOT EXISTS progresso (id INTEGER PRIMARY KEY AUTOINCREMENT, aluno_id INTEGER NOT NULL, aula_id INTEGER NOT NULL, UNIQUE(aluno_id, aula_id))`);
    });
});
...
// NOVIDADE: Rota para buscar o progresso de um aluno em todos os seus cursos
app.get('/api/alunos/:id/progresso', authenticateToken, (req, res) => {
    const { id } = req.params;
    // Esta consulta SQL é mais complexa: para cada curso que o aluno está inscrito,
    // ela conta o total de aulas e quantas delas estão na tabela de progresso.
    const sql = `
        SELECT
            c.id as curso_id,
            c.name as curso_nome,
            (SELECT COUNT(*) FROM aulas WHERE curso_id = c.id) as total_aulas,
            (SELECT COUNT(*) FROM progresso p JOIN aulas a ON p.aula_id = a.id WHERE p.aluno_id = ? AND a.curso_id = c.id) as aulas_concluidas
        FROM cursos c
...
app.post('/api/inscricoes', authenticateToken, (req, res) => { const { aluno_id, curso_id } = req.body; if (!aluno_id || !curso_id) { return res.status(400).json({ error: "ID do aluno e do curso são obrigatórios." }); } const sql = 'INSERT OR IGNORE INTO inscricoes (aluno_id, curso_id) VALUES (?, ?)'; db.run(sql, [aluno_id, curso_id], function(err) { if (err) { return res.status(500).json({ error: err.message }); } if (this.changes === 0) { return res.status(409).json({ message: "O aluno já está inscrito neste curso." }); } res.status(201).json({ message: "Inscrição realizada com sucesso!", inscricaoId: this.lastID }); }); });
app.post('/api/progresso', authenticateToken, (req, res) => { const { aluno_id, aula_id } = req.body; if (!aluno_id || !aula_id) { return res.status(400).json({ error: "ID do aluno e da aula são obrigatórios." }); } const sql = 'INSERT OR IGNORE INTO progresso (aluno_id, aula_id) VALUES (?, ?)'; db.run(sql, [aluno_id, aula_id], function(err) { if (err) { return res.status(500).json({ error: err.message }); } res.status(201).json({ message: "Progresso salvo com sucesso!" }); }); });
app.get('/api/alunos/:id/progresso', authenticateToken, (req, res) => { const { id } = req.params; const sql = `SELECT c.id as curso_id, c.name as curso_nome, (SELECT COUNT(*) FROM aulas WHERE curso_id = c.id) as total_aulas, (SELECT COUNT(*) FROM progresso p JOIN aulas a ON p.aula_id = a.id WHERE p.aluno_id = ? AND a.curso_id = c.id) as aulas_concluidas FROM cursos c JOIN inscricoes i ON c.id = i.curso_id WHERE i.aluno_id = ?`; db.all(sql, [id, id], (err, rows) => { if (err) { return res.status(500).json({ error: err.message }); } res.json({ message: "Progresso do aluno", data: rows }); }); });
...
app.get('/api/alunos/:aluno_id/cursos/:curso_id/aulas', authenticateToken, (req, res) => { const { aluno_id, curso_id } = req.params; const sql = ` SELECT a.id, a.titulo, a.tipo, a.conteudo, CASE WHEN p.id IS NOT NULL THEN 1 ELSE 0 END as concluida FROM aulas a LEFT JOIN progresso p ON a.id = p.aula_id AND p.aluno_id = ? WHERE a.curso_id = ?`; db.all(sql, [aluno_id, curso_id], (err, rows) => { if (err) { return res.status(500).json({ error: err.message }); } res.json({ message: "Aulas do curso com progresso", data: rows }); }); });
